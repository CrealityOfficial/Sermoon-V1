...
static uint8_t SetSysClock_PLL_HSE(uint8_t bypass)
{
  ...
  RCC_OscInitStruct.PLL.PLLM            = 8; // Expects an 8 MHz external clock by default. Redefine HSE_VALUE if not
  RCC_OscInitStruct.PLL.PLLN            = 144;           // VCO output clock = 336 MHz (1 MHz * 336)
  RCC_OscInitStruct.PLL.PLLP            = RCC_PLLP_DIV2; // PLLCLK = 84 MHz (336 MHz / 4)
  RCC_OscInitStruct.PLL.PLLQ            = 12;            // USB clock = 48 MHz (336 MHz / 7) --> OK for USB
