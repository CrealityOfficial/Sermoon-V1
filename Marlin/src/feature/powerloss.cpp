...
void PrintJobRecovery::check() {
  ...
  if (card.isMounted()) {
    ...
    // TERN_(DWIN_CREALITY_LCD, dwin_flag = true);
        ...
        /* check the valid of print-job-recovery-information that read from EEPROM */
        for(uint8_t i=0; i<5; i++)
        {
          if(info.valid())
          {
            HOTEND_LOOP() info.target_temperature[e] = info.target_temperature[e] == 0 ? \
                                                 201 : info.target_temperature[e];
            return;
          }
          else
          {
            BL24CXX::read(PLR_ADDR, (uint8_t*)&info, sizeof(info));
            SERIAL_ECHOLN("Warning: Read EEPROM error, check failed!");
          }
        }
      #else
        load();
...
void PrintJobRecovery::save(const bool force/*=false*/, const float zraise/*=0*/) {
  ...
  if (force
    ...
    info.feedrate = uint16_t(feedrate_mm_s * 60.0f);
      ...
      /* hotend temperature mustn't lower than EXTRUDE_MINTEMP */
      HOTEND_LOOP() info.target_temperature[e] = thermalManager.temp_hotend[e].target < EXTRUDE_MINTEMP ? \
                                                 EXTRUDE_MINTEMP : thermalManager.temp_hotend[e].target;
    #endif
    ...
    #ifdef EEPROM_PLR
      BL24CXX::write(PLR_ADDR, (uint8_t*)&info, sizeof(info));
    #else
      write();
    #endif
    // write();
...
void PrintJobRecovery::resume() {
  ...
  /* clh+ Extrude a section of material before continuing to print */
  // current_position[E_AXIS] += 20.0;
  // line_to_current_position(feedRate_t(3));
  ...
  sprintf_P(cmd, PSTR("G1 X%s Y%s F3000"),
    dtostrf(info.current_position.x * 9 / 10, 1, 3, str_1),
    dtostrf(info.current_position.y * 9 / 10, 1, 3, str_2)
  );
  gcode.process_subcommands_now(cmd);
  
  sprintf_P(cmd, PSTR("G1 X%s Y%s E6 F300"),
