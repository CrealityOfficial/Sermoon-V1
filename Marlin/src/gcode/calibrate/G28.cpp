...
void GcodeSuite::G28() {
  ...
  endstops.enable(true); // Enable endstops for next homing move
    ...
    const bool homeZ = parser.seen('Z'),
      ...
      // current_position[Z_AXIS] -= mbl.z_offset;
  ...
  if(!doZ) current_position[Z_AXIS] = lPosRecord[Z_AXIS];
  ...
  // TERN_(DWIN_CREALITY_LCD, DWIN_CompletedHoming());//caixiaoliang 20210326

  do_blocking_move_to_xy(0.0f,0.0f);//avoiding edge collision
  report_current_position();
    ...
    else if(waitway == 6)
    {
      if(AutohomeKey)
      {
        InforShowStatus = AutohomeKey = false;
          ...
          if(LanguageRecbuf != 0)
          {
            // rtscheck.RTS_SndData(ExchangePageBase + 28 + AxisPagenum, ExchangepageAddr);
            rtscheck.RTS_SndData(ExchangePageBase + 28, ExchangepageAddr);//caixiaoliang 20201204 modify
          }
          else
          {
            // rtscheck.RTS_SndData(ExchangePageBase + 84 + AxisPagenum, ExchangepageAddr);
            rtscheck.RTS_SndData(ExchangePageBase + 84, ExchangepageAddr);//caixiaoliang 20201204 modify
          }
        #endif
      }
      waitway = 0;
