...
extern uint8_t flag_system_error;
  ...
  void Temperature::PID_autotune(const float &target, const heater_ind_t heater, const int8_t ncycles, const bool set_result/*=false*/) {
    ...
    while (wait_for_heatup) {
      ...
      if ((ms - _MIN(t1, t2)) > (MAX_CYCLE_TIME_PID_AUTOTUNE * 60L * 1000L)) {
        // TERN_(DWIN_CREALITY_LCD, Popup_Window_Temperature(0));//caixiaoliang 20210326 modify
        #ifdef RTS_AVAILABLE
          #if 0
...
void Temperature::min_temp_error(const heater_ind_t heater) {
  ...
  float Temperature::get_pid_output_hotend(const uint8_t E_NAME) {
    const uint8_t ee = HOTEND_INDEX;
        ...
        float pid_output;
          ...
          {
            room_temperature_hotend = temp_hotend[0].celsius;
          }
          ...
          else if( PrinterStatusKey[1] == 2 && ((!temp_hotend[ee].target && pid_error >= -(room_temperature_hotend +5) ) ||(temp_hotend[ee].target && pid_error >= 0)))
          {
            if(LanguageRecbuf != 0)
            {
              // 9 for Cooling Done
  ...
  float Temperature::get_pid_output_bed() {
      ...
      const float max_power_over_i_gain = float(MAX_BED_POWER) / temp_bed.pid.Ki - float(MIN_BED_POWER),
        ...
        else if( PrinterStatusKey[1] == 2 && ((!temp_bed.target && pid_error >= -(room_temperature_hotbed +5) ) ||(temp_bed.target && pid_error >= 0)))
        {
          if(LanguageRecbuf != 0)
          {
            // 9 for Cooling Done
...
void Temperature::init() {
  ...
  delay(250);
    ...
    #define _TEMP_MAX_E(NR) do{ \
      const int16_t tmax = HEATER_ ##NR## _MAXTEMP; \
      temp_range[NR].maxtemp = tmax; \
      while (analog_to_celsius_hotend(temp_range[NR].raw_max, NR) > tmax) \
        temp_range[NR].raw_max -= TEMPDIR(NR) * (OVERSAMPLENR); \
    }while(0)
...
void Temperature::tick() {
  ...
  void Temperature::print_heater_states(const uint8_t target_extruder
    ...
    SERIAL_ECHOPAIR(" FAN0@:", thermalManager.fan_speed[0]);
    SERIAL_ECHOPAIR(" FAN1@:", 0);
  }
