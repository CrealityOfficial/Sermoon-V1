 ...
 */
/**
 * [HeartBeatProcess :heart beat main loop print]
 * @Author Creality
 * @Time   2021-09-26
 */
void HeartBeatMainLoop(void)
{
  ...
  if((millis() - lMs) > 2000)
  {
    ...
    SERIAL_ECHOLN("--------------------------------------------Heart beat main loop");
  }
}

/**
 * [HeartBeatProcess :heart beat idle loop print]
 * @Author Creality
 * @Time   2021-09-26
 */
void HeartBeatIdleLoop(void)
{
  static uint32_t lMs = millis();
  ...
  if((millis() - lMs) > 2000)
  {
    ...
    SERIAL_ECHOLN("--------------------------------------------Heart beat idle loop");
  }
}
...
void idle(TERN_(ADVANCED_PAUSE_FEATURE, bool no_stepper_sleep/*=false*/)) {
  ...
  // TERN(DWIN_CREALITY_LCD, DWIN_Update(), ui.update()); //caixiaoliang 20210325 modify
...
void setup() {
  ...
  #if ENABLED(DWIN_CREALITY_LCD)
    // delay(800);   // Required delay (since boot?) //caixiaoliang 20210325 modify
    // SERIAL_ECHOPGM("\nDWIN handshake ");
  ...
  #if ENABLED(DWIN_CREALITY_LCD)
    // Encoder_Configuration();//caixiaoliang 20210325 modify
    // HMI_Init();
    // HMI_StartFrame(true);
  #endif
 ...
 */
// static unsigned char databuf[6]={0x01,0x02,0x03,0x04,0x05,0x06};
void loop() {
  do {
    #if 1
