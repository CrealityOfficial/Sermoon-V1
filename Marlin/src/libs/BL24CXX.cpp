...
//IO·½ÏòÉèÖÃ
#define SDA_IN()  {FastIOPortMap[STM_PORT(digitalPin[IIC_EEPROM_SDA])]->MODER&=~(3<<(11*2));FastIOPortMap[STM_PORT(digitalPin[IIC_EEPROM_SDA])]->MODER|=0<<11*2;}  //PA11
#define SDA_OUT() {FastIOPortMap[STM_PORT(digitalPin[IIC_EEPROM_SDA])]->MODER&=~(3<<(11*2));FastIOPortMap[STM_PORT(digitalPin[IIC_EEPROM_SDA])]->MODER|=1<<11*2;}  //pA11 
